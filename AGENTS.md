...
- Backend entrypoint: `backend/cmd/h2v/main.go`.
...
- Installer defaults must not silently overwrite existing `/opt/h2v/.env` secrets.
- systemd unit paths must match `INSTALL_DIR=/opt/h2v`.
